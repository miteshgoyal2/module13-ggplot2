...
plot <- ggplot(data = diamonds.sample) # Just an empty space which can be plotted on.

# Draw a scatter plot (with point geometry) with for the `diamonds.sample` set,
# with the `carat` mapped to the x-position and `price` mapped to the y-position.

plot + geom_point(mapping = aes(x = carat, y = price))

# Draw the same plot as above, but color each of the points based on their clarity.
plot + geom_point(mapping = aes(x = carat, y = price, color = clarity))
